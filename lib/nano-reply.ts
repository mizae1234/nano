...
export interface BotMeta {
  botName?: string;    // "น้องนาโน"
  botPersona?: string; // "ค่ะ"
...
function headerBox(title: string, themeColor: string, botName: string) {
  return {
    ...
    layout: "vertical",
    contents: [
      { type: "text", text: botName, color: "#ffffff", size: "sm", weight: "bold" },
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        layout: "vertical",
        contents: [
          { type: "text", text: menuMessage || `เลือกสิ่งที่ต้องการ${botPersona} 😊`, size: "sm", color: "#555555", margin: "md" },
          { type: "text", text: "💡 พิมพ์ \"แจ้ง [ระบบ] ปัญหา\" เพื่อสร้าง ticket", size: "xxs", color: "#aaaaaa", margin: "sm", wrap: true },
        ],
        paddingAll: "16px",
      },
      footer: { type: "box", layout: "vertical", contents: actions, spacing: "sm", paddingAll: "16px" },
    },
  };
}
...
export function systemSelectFlex(systems: SystemInfo[], action: string = "แจ้ง", bot?: BotMeta) {
  const { botName, themeColor } = meta(bot);
  const buttons = systems.map((sys) => ({
    type: "button",
    action: { type: "message", label: `${sys.icon || "⚙️"} ${sys.name}`, text: `${action} [${sys.code}] ` },
  ...
  return {
    ...
    contents: {
      type: "bubble",
      header: headerBox("เลือกระบบที่ต้องการแจ้ง", themeColor, botName),
      body: { type: "box", layout: "vertical", contents: [{ type: "text", text: "กรุณาเลือกระบบ 👇", size: "sm", color: "#555555" }], paddingAll: "16px" },
      footer: { type: "box", layout: "vertical", contents: buttons, spacing: "sm", paddingAll: "16px" },
    },
  };
}

// ══════════════════════════════════════════════════════════════
//  TICKET CREATED (Standard)
// ══════════════════════════════════════════════════════════════

export function ticketCreatedFlex(ticket: TicketInfo, liffUrl: string, bot?: BotMeta) {
  const { botName, themeColor } = meta(bot);
  const ticketRef = ticket.systemPrefix ? `${ticket.systemPrefix}-${ticket.ticketNo}` : `#${ticket.ticketNo}`;

  return {
    type: "flex",
    altText: `✅ สร้าง Ticket ${ticketRef} สำเร็จ`,
    contents: {
      type: "bubble",
      header: headerBox("✅ สร้าง Ticket สำเร็จ", themeColor, botName),
      body: {
        type: "box",
        layout: "vertical",
        contents: [
          { type: "text", text: ticketRef, weight: "bold", size: "xl", color: themeColor },
          { type: "text", text: ticket.title, size: "md", wrap: true, margin: "md" },
      ...
      footer: {
        ...
        contents: [{ type: "button", action: { type: "uri", label: "ดูรายละเอียด", uri: liffUrl }, style: "primary", color: themeColor }],
      },
    },
  };
}
...
// ══════════════════════════════════════════════════════════════
//  TICKET CREATED WITH AI ANALYSIS ✨ (NEW)
// ══════════════════════════════════════════════════════════════

export function ticketCreatedWithAIFlex(
  ticket: TicketInfo,
  liffUrl: string,
  aiAnalysis: { emoji: string; suggestedCategory?: string },
  bot?: BotMeta
) {
  const { botName, themeColor } = meta(bot);
  const ticketRef = ticket.systemPrefix ? `${ticket.systemPrefix}-${ticket.ticketNo}` : `#${ticket.ticketNo}`;
  ...
  return {
    type: "flex",
    altText: `${aiAnalysis.emoji} AI สร้าง Ticket ${ticketRef} สำเร็จ`,
    contents: {
      type: "bubble",
        ...
        contents: [
          {
            ...
            contents: [
              { type: "text", text: ticketRef, weight: "bold", size: "xl", color: themeColor, flex: 3 },
              {
                ...
                backgroundColor: priorityColor, paddingAll: "6px", cornerRadius: "8px",
              },
            ],
          },
          { type: "text", text: ticket.title, size: "sm", wrap: true, margin: "md", weight: "bold" },
          { type: "separator", margin: "lg" },
          {
          ...
          { type: "text", text: "🤖 AI วิเคราะห์ priority & หมวดหมู่ให้อัตโนมัติ", size: "xxs", color: "#aaaaaa", margin: "lg", wrap: true },
        ],
        paddingAll: "16px",
      },
      footer: {
        ...
        contents: [
          { type: "button", action: { type: "uri", label: "ดูรายละเอียด Ticket", uri: liffUrl }, style: "primary", color: themeColor, height: "sm" },
...
export function ticketListFlex(tickets: TicketInfo[], liffBaseUrl: string, bot?: BotMeta) {
  const { botName, botPersona, themeColor } = meta(bot);
      ...
      contents: {
        ...
        body: {
          ...
          contents: [
            ...
            { type: "text", text: "พิมพ์ \"แจ้ง ปัญหา...\" เพื่อสร้าง Ticket ใหม่", size: "xxs", color: "#aaaaaa", margin: "md", wrap: true },
          ],
        },
      },
    };
...
export function ticketStatusFlex(ticket: TicketInfo, bot?: BotMeta) {
  ...
  return {
    type: "flex",
    altText: `สถานะ Ticket ${ticketRef}`,
    contents: {
      type: "bubble",
      ...
      body: {
        type: "box", layout: "vertical", paddingAll: "16px",
        contents: [
          { type: "text", text: ticket.title, weight: "bold", size: "md", wrap: true },
          { type: "separator", margin: "lg" },
          {
            type: "box", layout: "vertical", margin: "lg", spacing: "sm",
            contents: [
  ...
  return {
    ...
    contents: {
      ...
      body: {
        type: "box", layout: "vertical", paddingAll: "16px",
        contents: [
          { type: "text", text: `สวัสดี${botPersona} คุณ${displayName}! 👋`, weight: "bold", size: "lg", wrap: true },
          { type: "text", text: welcomeMessage || `ยินดีต้อนรับสู่ระบบ ${orgName} ${botPersona}`, size: "sm", color: "#555555", margin: "md", wrap: true },
          { type: "text", text: "พิมพ์ \"ช่วย\" เพื่อดูเมนูการใช้งาน", size: "sm", color: "#888888", margin: "md", wrap: true },
        ],
      },
      footer: {
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          ...
          { type: "text", text: `ฟีเจอร์ AI ใช้ได้เฉพาะแผน Pro ขึ้นไป${botPersona}`, size: "sm", color: "#555555", wrap: true, margin: "md" },
          { type: "separator", margin: "lg" },
          { type: "text", text: "• Pro ฿2,990/เดือน — AI + ไม่จำกัดผู้ใช้", size: "xs", color: "#555555", margin: "md" },
          { type: "text", text: "• Enterprise — Dedicated DB + SLA", size: "xs", color: "#555555", margin: "sm" },
        ],
      },
    },
  };
  ...
  return {
    ...
    contents: {
      ...
      body: {
        ...
        contents: [
          {
            type: "box", layout: "vertical", backgroundColor: "#f9fafb", paddingAll: "10px", cornerRadius: "8px",
            contents: [
          ...
          { type: "separator", margin: "lg" },
          { type: "text", text: shortAnswer, size: "sm", wrap: true, margin: "lg", color: "#333333" },
        ],
      },
      footer: {
        type: "box", layout: "vertical", paddingAll: "12px",
...
// ══════════════════════════════════════════════════════════════
//  URGENT ALERT (Push Notification) 🚨 (NEW)
// ══════════════════════════════════════════════════════════════

export function urgentAlertFlex(ticket: TicketInfo, reporterName: string, liffUrl: string, bot?: BotMeta) {
  const { botName } = meta(bot);
  const ticketRef = ticket.systemPrefix ? `${ticket.systemPrefix}-${ticket.ticketNo}` : `#${ticket.ticketNo}`;

  return {
    type: "flex",
    altText: `🚨 URGENT! ${ticketRef}: ${ticket.title}`,
    contents: {
      type: "bubble",
      ...
      body: {
        ...
        contents: [
          {
            type: "box", layout: "horizontal", backgroundColor: "#fef2f2", paddingAll: "10px", cornerRadius: "8px",
            contents: [
              { type: "text", text: ticketRef, weight: "bold", size: "xl", color: "#ef4444", flex: 1 },
              { type: "text", text: "🔴 URGENT", size: "xs", color: "#ef4444", align: "end" as const, flex: 0 },
            ],
          },
          { type: "text", text: ticket.title, weight: "bold", size: "md", wrap: true, margin: "md" },
          { type: "separator", margin: "lg" },
          {
            type: "box", layout: "vertical", margin: "md", spacing: "sm",
            contents: [
      ...
      footer: {
        ...
        contents: [
          { type: "button", action: { type: "uri", label: "🔍 ดูรายละเอียดทันที", uri: liffUrl }, style: "primary", color: "#ef4444", height: "sm" },
...
export function quickActionFlex(ticket: TicketInfo, liffUrl: string, bot?: BotMeta) {
  const { botName, themeColor } = meta(bot);
  const ticketRef = ticket.systemPrefix ? `${ticket.systemPrefix}-${ticket.ticketNo}` : `#${ticket.ticketNo}`;
  ...
  return {
    type: "flex",
    altText: `⚡ Quick Action — ${ticketRef}`,
    contents: {
      type: "bubble",
            ...
            contents: [
              {
                ...
                backgroundColor: statusColor[ticket.status] || "#6b7280", paddingAll: "6px", cornerRadius: "8px",
              },
              { type: "filler" },
      ...
      footer: {
        type: "box", layout: "vertical", spacing: "sm", paddingAll: "12px",
        contents: [
          { type: "button", action: { type: "uri", label: "📝 แก้ไข / อัปเดต", uri: liffUrl }, style: "primary", color: themeColor, height: "sm" },
          {
