...
export type NanoAction =
  ...
  | { action: "FOLLOW_TICKET"; ticketNo: string; systemPrefix?: string; isUnfollow?: boolean }
...
const UNFOLLOW_WORDS = ["ยกเลิกติดตาม", "เลิกติดตาม", "unfollow", "หยุดติดตาม"];
...
export function parseNanoCommand(
  ...
  // ─── 2. ติดตาม / ยกเลิกติดตาม ──────────────────────────────
  const unfollowCheck = startsWith(text, UNFOLLOW_WORDS);
  if (unfollowCheck.match) {
    const afterCommand = text.slice(unfollowCheck.keyword.length).trim();
    const prefixMatch = afterCommand.match(/^#?([A-Z]{2,6})-(\d+)$/i);
    if (prefixMatch) {
      return { action: "FOLLOW_TICKET", ticketNo: prefixMatch[2], systemPrefix: prefixMatch[1].toUpperCase(), isUnfollow: true };
    }
    const numMatch = afterCommand.match(/^#?(\d+)/);
    if (numMatch) return { action: "FOLLOW_TICKET", ticketNo: numMatch[1], isUnfollow: true };
    return { action: "SHOW_MENU" };
  }
  ...
  if (followCheck.match) {
    ...
    const prefixMatch = afterCommand.match(/^#?([A-Z]{2,6})-(\d+)$/i);
    if (prefixMatch) {
      return { action: "FOLLOW_TICKET", ticketNo: prefixMatch[2], systemPrefix: prefixMatch[1].toUpperCase(), isUnfollow: false };
    }
    const numMatch = afterCommand.match(/^#?(\d+)/);
    if (numMatch) return { action: "FOLLOW_TICKET", ticketNo: numMatch[1], isUnfollow: false };
    return { action: "SHOW_MENU" };
  }
